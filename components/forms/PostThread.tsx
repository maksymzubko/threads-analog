...
function PostThread({
  ...
  const uploadFiles = async () => {
    ...
    if (needUpload.length > 0) {
      const imgResponse = await startUpload(
        needUpload.map((f) => f.file ?? new File([], ''))
      )
  ...
  return (
    <div
      className={
        'flex items-start mt-10 gap-4 border-dark-3 p-5 rounded-2xl border-[1px]'
      }
    >
      ...
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit(
            isComment ? onSubmitComment : onSubmitThread
          )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className={'flex flex-col w-full gap-3 relative'}>
                <FormControl
                  className={
                    'no-focus border border-dark-4 bg-dark-3 text-light-1 max-h-[400px] min-h-[250px]'
                  }
                >
                  <CustomTextField
                    search={field.value}
          ...
          <div className={'flex justify-between'}>
            ...
            <Button
              ...
              disabled={
                ...
                form.formState.isSubmitting ||
                (form.formState.isSubmitSuccessful && !onHome && !isComment)
              }
