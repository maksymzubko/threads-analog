      ...
      <>
        {listOfValues.map((v) => {
          if (!!v.match(/@\[[\S]+\]\([\S]+\)/g)) {
            ...
            const id = data?.at(2) ?? ''
            const user = mentions.find((m: any) => m.user._id.toString() === id)
              ?.user
    ...
    <>
      <article
        ...
        className={`flex w-full flex-col ${
          isComment
            ? `px-0 xs:px-7 ${!child?.author && 'border-b-[1px] border-dark-3'}`
            : 'bg-dark-2 p-7 rounded-xl'
        }`}
      >
        <div
          className={`${isComment && !isChild && 'mt-5'} ${
            isComment && (child ? 'mb-2' : 'mb-5')
        ...
        >
          <div className={'flex w-full flex-1 flex-row gap-4'}>
            <div className={'flex flex-col items-center'}>
              ...
              {(comments.length > 0 || isMain) && (
                <div className={'thread-card_bar'} />
              )}

              {currentUserId === author.id && <EditCardForm id={id} />}
            </div>
            <div className={`flex w-full flex-col`}>
              <div className={'w-fit flex gap-2 text-gray-1 items-center'}>
                ...
                >
                  <Link
                    href={`/profile/@${author.username}`}
                    className={`flex gap-2 text-gray-1 items-center text-ellipsis [&>*]:whitespace-nowrap [&>*]:overflow-hidden [&>*]:overflow-ellipsis [&>*]:max-w-[60px] ${
                      isComment ? '' : 'xs:[&>*]:max-w-[100px]'
                    } sm:[&>*]:max-w-[100px] md:[&>*]:max-w-[120px]`}
                  >
                    <h4
                      className={
        ...
        {index === 2 && child && (
          <Link href={`/thread/${parentId}`}>
            <div className={'text-light-2 cursor-pointer mb-4'}>Show more</div>
          </Link>
        )}
          ...
          <p className="text-subtle-medium text-gray-1 mt-5 flex items-center">
            ...
            {community && (
              <Image
                ...
                height={14}
                className={
                  'ml-1 h-[14px] hidden md:block rounded-full object-cover'
                }
              />
            )}
          </p>
