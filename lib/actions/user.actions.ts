    ...
    if (existedUser)
      return {
        error: {
          ...
          error: {
            type: 'custom',
            message: 'User with same name is already exist!',
          },
        },
      }
...
export async function fetchUser(userId: string) {
  try {
    ...
    return await User.findOne({
      $or: [{ id: userId }, { username: userId.replace('%40', '') }],
    })
  } catch (error: any) {
...
export async function fetchUsers({
  ...
  try {
    ...
    } else query.$or = [{ username: { $regex: regex } }]
...
const findReplies = async (childThreadIds: any[], userId: string) => {
  ...
  replies.forEach((r) => {
    result.push({
      ...r?._doc,
      type: 'reply',
      date: new Date(r?._doc.createdAt).getTime(),
    })
  })
  return result
}
    ...
    th.likes.forEach((l: any) => {
      if (l.user._id.toString() !== userId.toString())
        result.push({
          ...
          user: l.user,
          type: 'like',
          date: new Date(l.createdAt).getTime(),
        })
    })
  })
  ...
  threadsWithTags.forEach((th) => {
    th.mentioned.forEach((l: any) => {
      result.push({
        ...
        user: l.user,
        type: 'tag',
        date: new Date(th?._doc.createdAt).getTime(),
      })
    })
  })
  return result
...
export async function getActivity(
  ...
  try {
    ...
    if (type === 'reply')
      return (await findReplies(childThreadIds, userId)).sort(
        (a, b) => b.date - a.date
      )

    if (type === 'like')
      return (await findLikes(childThreadIds, userId)).sort(
        (a, b) => b.date - a.date
      )

    if (type === 'tag')
      return (await findTags(childThreadIds, userId)).sort(
        (a, b) => b.date - a.date
      )
    ...
    if (type === 'all') {
      ...
      const sorted = [...replies, ...likes, ...fixedTags].sort(
        (a, b) => b.date - a.date
      )
